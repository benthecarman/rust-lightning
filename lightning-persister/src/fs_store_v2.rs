...
impl KVStoreSync for FilesystemStoreV2 {
	...
	fn write(
		...
		self.inner.write_impl(primary_namespace, secondary_namespace, key, &buf, true, true)
	}
	...
	fn list(
		...
		self.inner.list_impl(primary_namespace, secondary_namespace, true, false)
	}
}
...
impl PaginatedKVStoreSync for FilesystemStoreV2 {
	fn list_paginated(
		...
		self.inner.list_paginated_impl(primary_namespace, secondary_namespace, page_token)
	}
}
...
impl KVStore for FilesystemStoreV2 {
	fn read(
		...
		Arc::clone(&self.inner).read_async(primary_namespace, secondary_namespace, key, true)
	}
	...
	fn list(
		...
		self.inner.list_async(primary_namespace, secondary_namespace, true, false)
	}
}
	...
	) -> impl Future<Output = Result<PaginatedListResponse, lightning::io::Error>> + 'static + Send
	{
		...
		async move {
			tokio::task::spawn_blocking(move || {
				inner.list_paginated_impl(&primary_namespace, &secondary_namespace, page_token)
			})
			.await
...
mod tests {
	use super::*;
	use crate::fs_store_common::{
		format_page_token, parse_page_token, EMPTY_NAMESPACE_DIR, PAGE_SIZE,
	};
