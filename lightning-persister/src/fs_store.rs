...
use crate::fs_store_common::FilesystemStoreState;
...
impl FilesystemStore {
	...
	pub fn new(data_dir: PathBuf) -> Self {
		Self { inner: Arc::new(FilesystemStoreState::new(data_dir)) }
	}
...
impl KVStoreSync for FilesystemStore {
	fn read(
		...
		self.inner.read_impl(primary_namespace, secondary_namespace, key, false)
	}

	fn write(
		&self, primary_namespace: &str, secondary_namespace: &str, key: &str, buf: Vec<u8>,
	) -> Result<(), lightning::io::Error> {
		self.inner.write_impl(primary_namespace, secondary_namespace, key, &buf, false, false)
	}

	fn remove(
		&self, primary_namespace: &str, secondary_namespace: &str, key: &str, lazy: bool,
	) -> Result<(), lightning::io::Error> {
		self.inner.remove_impl(primary_namespace, secondary_namespace, key, lazy, false)
	}

	fn list(
		&self, primary_namespace: &str, secondary_namespace: &str,
	) -> Result<Vec<String>, lightning::io::Error> {
		self.inner.list_impl(primary_namespace, secondary_namespace, false, true)
	}
}
...
impl KVStore for FilesystemStore {
	fn read(
		...
		Arc::clone(&self.inner).read_async(primary_namespace, secondary_namespace, key, false)
	}

	fn write(
		&self, primary_namespace: &str, secondary_namespace: &str, key: &str, buf: Vec<u8>,
	) -> impl Future<Output = Result<(), lightning::io::Error>> + 'static + Send {
		Arc::clone(&self.inner).write_async(
			primary_namespace,
			secondary_namespace,
			key,
			buf,
	...
	fn remove(
		...
		Arc::clone(&self.inner).remove_async(
			primary_namespace,
			secondary_namespace,
			key,
			lazy,
...
mod tests {
	...
	#[test]
	fn test_empty_namespace_variations() {
		use lightning::util::persist::KVStoreSync;

		let mut temp_path = std::env::temp_dir();
		temp_path.push("test_empty_namespace_variations_v1");
		let store = FilesystemStore::new(temp_path.clone());

		let data = vec![42u8; 32];

		// Both namespaces empty
		KVStoreSync::write(&store, "", "", "key1", data.clone()).unwrap();
		assert_eq!(KVStoreSync::read(&store, "", "", "key1").unwrap(), data);
		assert!(KVStoreSync::list(&store, "", "").unwrap().contains(&"key1".to_string()));

		// Primary non-empty, secondary empty
		KVStoreSync::write(&store, "primary", "", "key2", data.clone()).unwrap();
		assert_eq!(KVStoreSync::read(&store, "primary", "", "key2").unwrap(), data);
		assert!(KVStoreSync::list(&store, "primary", "").unwrap().contains(&"key2".to_string()));

		// Both non-empty
		KVStoreSync::write(&store, "primary", "secondary", "key3", data.clone()).unwrap();
		assert_eq!(KVStoreSync::read(&store, "primary", "secondary", "key3").unwrap(), data);
		assert!(KVStoreSync::list(&store, "primary", "secondary")
			.unwrap()
			.contains(&"key3".to_string()));

		// Verify the different namespace combinations don't collide
		assert_eq!(KVStoreSync::list(&store, "", "").unwrap().len(), 1);
		assert_eq!(KVStoreSync::list(&store, "primary", "").unwrap().len(), 1);
		assert_eq!(KVStoreSync::list(&store, "primary", "secondary").unwrap().len(), 1);

		// Verify v1 directory structure: variable depth
		assert!(temp_path.join("key1").exists());
		assert!(temp_path.join("primary").join("key2").exists());
		assert!(temp_path.join("primary").join("secondary").join("key3").exists());

		// Remove and verify
		KVStoreSync::remove(&store, "", "", "key1", false).unwrap();
		assert!(KVStoreSync::list(&store, "", "").unwrap().is_empty());
		KVStoreSync::remove(&store, "primary", "", "key2", false).unwrap();
		assert!(KVStoreSync::list(&store, "primary", "").unwrap().is_empty());
		KVStoreSync::remove(&store, "primary", "secondary", "key3", false).unwrap();
		assert!(KVStoreSync::list(&store, "primary", "secondary").unwrap().is_empty());
	}
